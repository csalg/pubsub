...
void opIndex::match(const Pub &pub, int &matchSubs, const vector<Sub>& subList)
{
    initCounter((vector<IntervalSub> &) subList);
...
void opIndex::match(const Pub &pub, int &matchSubs, const vector<IntervalSub>& subList)
{
    initCounter((vector<Sub> &) subList);
    for (int i = 0; i < pub.size; i++)
    {
        int piv_att = pub.pairs[i].att;
        if (!isPivot[piv_att])
