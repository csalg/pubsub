    ...
    for (int pub_iter = 0; pub_iter < gen.pubList.size(); pub_iter++)
    {
        ...
        for (int i = 0; i < gen.subList.size(); i++)
        {
            ...
            for (int j = 0; matchFlag && j < gen.subList[i].size; j++)
            {
                ...
                bool flag = false;
                for (int k = 0; k < pub.size; k++)
                    if (pub.pairs[k].att == cnt.att && pub.pairs[k].value >= cnt.lowValue && pub.pairs[k].value <= cnt.highValue)
                        flag = true;
                if (!flag)
                    matchFlag = false;
            }
...
double Util::mean(const vector<double> &a)
{
    double sum = 0;
...
std::string Util::AddZero(std::string str, int len)
{
    std::string result;
    int strLen = str.size();
...
//void Util::Maopao(int* array,int begin,int last)
...
string Util::PrintHead(int operation)
{
    string ostr = "";
...
void Util::ListResult(int matchNo, double total, double avg, double max, double min, double std)
{
    cout << "    Total Matched Subs: " << matchNo << endl;
    cout << "    Tot time(s): " << total / 1000 << endl;
    cout << "    Avg time(s): " << avg / 1000 << endl;
    cout << "    Max time(s): " << max / 1000 << endl;
    cout << "    Min time(s): " << min / 1000 << endl;
    cout << "    Std time(s): " << std / 1000 << endl;
    cout << endl;
}
...
void Util::OutputStatistics(string method, int pubs, int subs, int atts, int comAtts, int cons, double wid, int subDis, int pubDis, double zipf, int testId, vector<int> matSubList, vector<double> matTimeList, vector< vector<int> > priList, vector< vector<double> > detTimeList, int freq[])
{
    ...
    string commonTitle = method + "\tParameters: \n  Pubs:\t" + Util::Int2String(pubs)
                         ...
                         + "\n  Cons:\t" + Util::Int2String(cons) + "\n  Wid:\t" + Util::RemoveLastZero(Util::Double2String(wid))
    ...
    for (int i = 0; i<pubs; i++)
    {
        ...
        for (unsigned int j = 0; j<detTimeList[i].size(); j++)
        {
            ...
            detTimeText += Util::Int2String(priList[i].at(j)) + "\t" + Util::RemoveLastZero(Util::Double2String(detTimeList[i].at(j))) + "\r";
        }
...
int Util::BiSearch(vector<double> array, double target)
{
    /*
    ...
    */
    ...
    int mid = 0;
    if (target < array.at(low))
        mid = 0;
    else if (target > array.at(high))
        mid = high + 1;
    else
    {
        while (low <= high)
...
vector<int> Util::SortAtt(int fre[], int atts)
{
    int temp;
    int index[atts];
    vector<int> result;
